...
export default function AdminTeamDetailsPage() {
  ...
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        flash(`Failed to load team details: ${getErrorMessage(err)}`, 'error');
  ...
  const handleUpdateDetails = async () => {
    try {
      ...
      flash(`Failed to update team details: ${getErrorMessage(err)}`, 'error');
    }
  };
    ...
    ask(
      ...
      async () => {
        try {
          ...
          setNewLeader('');
          flash(`Leader changed to ${newLeader}.`, "success");
        } catch (err) {
          flash(`Failed to change leader: ${getErrorMessage(err)}`, 'error');
        }
      },
      "Change Team Leader"
    );
  ...
  const handleRemoveMember = (memberUsername) => {
    ask(
      `Remove ${memberUsername} from the team?`,
      async () => {
        try {
          ...
          flash(`${memberUsername} removed from team.`, "success");
        } catch (err) {
          flash(`Failed to remove member: ${getErrorMessage(err)}`, 'error');
        }
      },
      "Remove Member"
    );
  ...
  const handleAddMember = async () => {
    ...
    try {
        ...
        flash(`Failed to add member: ${getErrorMessage(err)}`, 'error');
    }
  };

  const handleDeleteTeam = () => {
    ask(
      "Are you sure you want to DELETE this team? All tasks will be lost!",
      async () => {
        try {
  ...
  return (
    <div>
      <button onClick={() => navigate('/admin/teams')} className="mb-4 text-text-muted hover:text-text-main">
        ← Back to Teams
      </button>
      ...
      <div className="bg-bg-card shadow-md rounded-lg p-6 mb-6 border-l-4 border-brand">
          ...
          <div className="w-full">
            {isEditing ? (
              <div className="space-y-3 max-w-lg">
                <input 
                  className="text-2xl font-bold border p-2 rounded w-full bg-bg-card text-text-main"
                  value={editForm.name}
                  onChange={(e) => setEditForm({...editForm, name: e.target.value})}
                />
                <textarea 
                  className="text-text-muted border p-2 rounded w-full bg-bg-card"
                  rows="2"
              ...
              <>
                <h1 className="text-3xl font-bold text-text-main flex items-center">
                    {team.name}
                    <button onClick={() => setIsEditing(true)} className="ml-3 text-text-muted hover:text-brand" title="Edit Details"><Edit2 className="w-5 h-5" /></button>
                </h1>
        ...
        <div className="mt-6 flex flex-wrap gap-6 text-sm text-text-muted border-t pt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
            {/* LEADER CARD */}
            <div className="bg-bg-card shadow rounded-lg p-6">
                <h3 className="font-bold text-text-muted mb-4 uppercase text-xs tracking-wider">Team Leader</h3>
                {!isChangingLeader ? (
                    <div className="flex justify-between items-center">
                        <div className="flex items-center">
                            <div className="h-10 w-10 rounded-full bg-brand/20 flex items-center justify-center text-brand font-bold mr-3">{team.leader_id.substring(0, 2).toUpperCase()}</div>
                            <Link to={`/admin/users/${team.leader_id}`} className="font-medium text-text-main hover:text-brand hover:underline">{team.leader_id}</Link>
                        </div>
                        <button onClick={() => setIsChangingLeader(true)} className="text-xs text-brand hover:underline">Change</button>
                    </div>
                ) : (
                    <div className="space-y-3">
                        ...
                        <div className="flex space-x-2 text-xs">
                             <button onClick={handleChangeLeader} disabled={!newLeader} className="bg-primary text-text-on-primary px-2 py-1 rounded disabled:opacity-50">Confirm</button>
            ...
            {/* MEMBERS LIST */}
            <div className="bg-bg-card shadow rounded-lg p-6">
                <h3 className="font-bold text-text-muted mb-4 uppercase text-xs tracking-wider">Team Members ({team.member_ids.length})</h3>
                    ...
                    {team.member_ids.map(member => (
                        <li key={member} className="py-3 flex justify-between items-center">
                            <div className="flex items-center">
                                ...
                                <Link to={`/admin/users/${member}`} className="text-sm font-medium text-text-main hover:text-brand hover:underline">{member}</Link>
                            </div>
                        </li>
                    ))}
                    {team.member_ids.length === 0 && <li className="text-sm text-text-muted italic">No members yet.</li>}
                </ul>
            </div>
        </div>
        ...
        <div className="md:col-span-2">
            <div className="bg-bg-card shadow rounded-lg overflow-hidden">
                <div className="px-6 py-4 border-b border-bg-card/20 bg-bg-card/70 flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-3 sm:space-y-0">
                    <h3 className="font-bold text-text-muted uppercase text-xs tracking-wider">Team Tasks</h3>
                    <div className="flex items-center space-x-2">
                        <div className="relative">
                            <select value={taskStatusFilter} onChange={(e) => setTaskStatusFilter(e.target.value)} className="appearance-none bg-bg-card border border-white-400 text-text-main py-1 px-3 pr-8 rounded leading-tight focus:outline-none focus:border-brand text-xs font-medium cursor-pointer">
                        ...
                        <button onClick={() => setSortByDue(!sortByDue)} className={`flex items-center px-3 py-1 rounded border text-xs font-medium transition-colors ${sortByDue ? 'bg-brand/20 border-brand text-brand' : 'bg-bg-card border-gray-300 text-text-muted hover:bg-gray-50'}`}><ArrowUpDown className="w-3 h-3 mr-1" /> Sort by Due Date</button>
                    </div>
                </div>
                
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-bg-card/90">
                            <tr>
                        ...
                        <tbody className="bg-bg-card divide-y divide-gray-200">
                            {tasks.length > 0 ? (
                                tasks.map(task => (
                                    <tr key={task.id} className="hover:bg-bg-card/70">
                                        <td className="px-6 py-4 text-sm font-bold">
                                            <Link to={`/admin/tasks/${task.id}`} className="text-brand hover:text-primary-hover hover:underline">{task.title}</Link>
                                        </td>
                                        <td className="px-6 py-4 text-sm text-text-muted">
                                            <Link to={`/admin/users/${task.assigned_to}`} className="hover:underline hover:text-brand">{task.assigned_to}</Link>
                                        </td>
                                ...
                                <tr><td colSpan="5" className="px-6 py-8 text-center text-text-muted">No tasks found matching current filters.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
